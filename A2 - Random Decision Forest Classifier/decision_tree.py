...
class DecisionTree:
    ...
    def _entropy(self, y):
        ...
        for i in range(self.num_classes):
            if counts[i] == 0:
                ...
                entropy = ((counts[i]/N)*(np.log2(counts[i]/N))) + entropy
        entropy = -entropy
        # ====================================================
    ...
    def _find_split(self, X, y, H_data):
        ...
        for ii in range(unique_values.shape[0] - 1):
            ...
            Nleft = current_split_index
            yleft = y[0:Nleft]
            Nright = N-Nleft
            yright = y[Nleft:]
            H_left = self._entropy(yleft)
            H_right = self._entropy(yright)

            current_information_gain = H_data - (Nleft/N)*(H_left) - (Nright/N)*(H_right)
