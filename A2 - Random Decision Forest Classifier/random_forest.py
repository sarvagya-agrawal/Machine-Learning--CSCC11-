...
class RandomForest:
    ...
    def build(self, X, y):
        ...
        for tree_i in range(self.num_trees):
            ...
            data_shuffler = self.rng.permutation(N)
            data_shuffled_X = X[data_shuffler]
            data_shuffled_y = y[data_shuffler]

            feature_shuffler = self.rng.permutation(D)
            feature_shuffled_X = data_shuffled_X[:, feature_shuffler]
            
            feat_ids = feature_shuffler[0:num_features_per_tree]
            
            X_sub = feature_shuffled_X[0:num_data_per_tree, 0:num_features_per_tree] 
            y_sub = data_shuffled_y[0:num_data_per_tree]
            # ====================================================
