...
class RBFRegression():
    ...
    def predict(self, X):
        ...
        for i in range(self.K):
            mat_B = np.append(mat_B, self._rbf_2d(X, i), axis = 1)
        y = np.dot(mat_B, self.parameters)
        return y
        # ====================================================
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coef):
        ...
        mat_B = np.ones((N,1))
        for i in range(self.K):
            mat_B = np.append(mat_B, self._rbf_2d(train_X, i), axis = 1)
        mat_BT = np.transpose(mat_B)
        B_TB = np.dot(mat_BT, mat_B)
        term1 = np.linalg.inv(B_TB + lambdaI)
        term2 = np.dot(mat_BT, train_Y)
