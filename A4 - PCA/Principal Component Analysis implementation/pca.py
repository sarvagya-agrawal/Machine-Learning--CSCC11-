...
class PCA:
    ...
    def plot_eigenvalues(self, savefig=False):
        ...
        eigenvalues = self.w
        print(eigenvalues)
        D_labeling = np.arange(1, self.D + 1)
        plt.plot(D_labeling, eigenvalues)
        plt.axis([0, self.D, 0, eigenvalues[0] + 1])
    ...
    def plot_subspace_variance(self, savefig=False):
        ...
        for i in range(self.D):
            new_num = self.w[i] + new_num
            new_term = new_num/sum(self.w)
            fractions = np.append(fractions, new_term)
        print(fractions)
        D_labeling = np.arange(0, self.D+1)
        plt.plot(D_labeling, fractions)
        plt.axis([0, self.D, 0, 1.1])
